...
/**
 * CAMT Parser Demo Script (TypeScript version)
 * 
...
import { parseCamt053, BankStatementDocument, Statement, Transaction, TransactionDetail } from 'camt-parser';

// Path to the sample CAMT.053 XML file
const sampleFilePath = join(__dirname, 'sample-camt053.xml');
...
async function processCamtFile(): Promise<void> {
  try {
    console.log('Reading CAMT.053 XML file...');
    const xmlContent = readFileSync(sampleFilePath, 'utf-8');
    ...
    result.statements.forEach((statement: Statement, index: number) => {
      console.log(`\n=== Statement #${index + 1} ===`);
      ...
      statement.transactions.forEach((tx: Transaction, txIndex: number) => {
        ...
        if (tx.details.length > 0) {
          const detail: TransactionDetail = tx.details[0];
          console.log('\nTransaction Details:');
    ...
    const jsonOutputPath = join(__dirname, 'sample-camt053-parsed.json');
